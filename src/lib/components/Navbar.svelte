...
<header class="relative z-50">
	<div class="flex h-20 w-full flex-row items-center justify-between gap-4 bg-white p-4 px-6 shadow-md">
	...
	{#if isMobileMenuOpen}
		<div class="absolute top-full left-0 w-full bg-white shadow-md md:hidden">
			<ul class="flex flex-col items-center gap-4 p-4">
				...
				{#if !session}
					<li>
						<a class="text-xs font-light uppercase" href="/login" on:click={closeMobileMenu}>Login</a>
					</li>
				{/if}
				{#if session && session.user}
					<li>
						<a class="text-xs font-light uppercase" href="/logout" on:click={closeMobileMenu}>Logout</a>
					</li>
					<li>
						<a class="text-xs font-light uppercase" href="/control" on:click={closeMobileMenu}>
							Door Control
						</a>
					</li>
					{#if profile && profile.site_admin}
						<li>
							<a
								...
								on:click={closeMobileMenu}>Logs</a>
						</li>
						<li>
							<a class="text-xs font-light uppercase" href="/admin/invite" on:click={closeMobileMenu}>
								Invite
							</a>
						</li>
						<li>
							<a class="text-xs font-light uppercase" href="/admin/users" on:click={closeMobileMenu}>
								Users
							</a>
						</li>
					{/if}
